    ...
    <div className={cx('playlist')} style={style} data-testid="playlist">
      ...
      <div className={cx('videos-container')} data-testid="videos-container">
        {videos.map((video) => (
          ...
          >
            <div className={cx('playlist-item-thumbnail-container')} data-testid={`thumbnail-container-${video.id}`}>
              <img src={video.thumbnail} alt={video.title} data-testid={`thumbnail-${video.id}`} />
              {isSelected(video.id) && <div className={cx('now-playing-overlay')} data-testid={`now-playing-overlay-${video.id}`}>Now playing</div>}
            </div>
