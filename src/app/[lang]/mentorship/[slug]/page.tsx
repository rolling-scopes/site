...
export async function generateMetadata({ params }: PageProps<'/[lang]/mentorship/[slug]'>): Promise<Metadata> {
...
export async function generateStaticParams({ params: { lang } }: { params: Awaited<PageProps<'/[lang]/mentorship/[slug]'>['params']> }) {
  const locale = resolvePageLocale(lang);

  return await pageStore.loadPagesMetadata(PAGE_TYPE.MENTORSHIP_COURSE, locale);
}

export default async function MentorshipRoute({ params }: PageProps<'/[lang]/mentorship/[slug]'>) {
  const { slug, lang } = await params;
  const locale = resolvePageLocale(lang);
