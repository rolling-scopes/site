...
function processRouteNames(routes) {
  return Object.keys(routes).map((r) => {
    console.log(
      'before = ',
      r,
      'result = ',
      `/${r.replace(/routes/, '').replaceAll('.', '')}/`.replace('//', '/'),
    );
    return `/${r.replace(/routes/, '').replaceAll('.', '/')}/`.replace('//', '/');
  });
}
...
async function run() {
  ...
  queuedLinks = processRouteNames(build.routes);

  console.log('links', queuedLinks);
  processQueue();
}

async function processQueue() {
  // console.log({ queuedLinks });
...
async function crawlLink(pathname) {
  ...
  console.log(`http://localhost${pathname}`);

  console.log(`Rendering path: ${pathname} (${queuedLinks.length} known link(s) remaining)`);
  ...
  if (args.singleFetch) {
    let dataPath = pathname === '/' ? '/_root.data' : `${pathname.replace(/\/$/, '')}.data`;
  ...
  for (let a of root.querySelectorAll('a')) {
    ...
    if (!crawledLinks.has(href) && !queuedLinks.includes(href) && args.routes == null) {
